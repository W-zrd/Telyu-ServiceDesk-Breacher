...
@click.option('--manual', is_flag=True, help='Manually input bearer token')
...
@cli.command()
@click.option('--id', 'ticket_id', type=int, required=True, help='Ticket ID to retrieve')
@click.option('--format', 'output_format', 
              type=click.Choice(['json', 'detail']), 
              default='detail',
              help='Output format')
def ticket(ticket_id, output_format):
    auth = TelUAuth()
    ...
    click.echo(f"{Fore.CYAN}🎫 Fetching ticket #{ticket_id}...{Style.RESET_ALL}")
    
    try:
        ticket_data = api.get_ticket_detail(ticket_id)
